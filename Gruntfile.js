...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    yaml2properties: {
      ...
      middleware_options: {
        options: {
          disableDest: true,
          middleware: function(response, properties) {
            grunt.file.write('tmp/middleware_options/response.properties', response);
            grunt.file.write('tmp/middleware_options/properties.properties', properties.join('\n'));
          }
        },
        files: {
          'tmp/middleware_options/middleware.properties': ['test/fixtures/middleware.yml']
        }
      }
    },
  ...
  grunt.registerTask('test', ['clean', 'yaml2properties', 'nodeunit']);
